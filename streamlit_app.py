...
def generate_keyboard_html(low_midi, high_midi):
    ...
    # 건반 너비 계산 (전체 폭을 흰 건반 개수로 나눔)
    key_width_percent = 100 / total_white_keys
    ...
    for midi in all_midi_notes:
        ...
        if is_white:
            ...
            white_keys_html += f"""
            <div class="white-key {'highlighted-white' if is_highlighted else ''}" style="width: calc(100% / {total_white_keys});">
                {label_text}
            </div>
            """
            ...
            # 검은 건반의 위치 계산
            # 현재까지 그려진 흰 건반 개수를 기준으로 검은 건반의 위치를 계산합니다.
            
            # 검은 건반은 해당 흰 건반 사이의 정중앙에 위치해야 합니다.
            # 검은 건반의 왼쪽 시작점 = (이전 흰 건반의 오른쪽 끝) - (검은 건반 너비의 절반)
            
            # 이전 흰 건반의 인덱스 (C#일 경우 C의 인덱스, D#일 경우 D의 인덱스)
            # 흰 건반은 0부터 시작하므로 white_key_index - 1이 이전 건반의 인덱스입니다.
            # 검은 건반은 흰 건반 사이에 껴있으므로, 시작점은 white_key_index - 1에 해당합니다.
            
            # 검은 건반이 위치해야 할 시작 위치 (흰 건반 기준)
            black_start_index = white_key_index - 1 
            
            # 흰 건반 하나 너비 (퍼센트)
            key_width_pc = 100 / total_white_keys
            
            # 검은 건반의 너비 (퍼센트)
            black_key_width_pc = key_width_pc * 0.6
            
            # 검은 건반의 왼쪽 위치 계산 (흰 건반 시작점 + (흰 건반 너비 / 2) - (검은 건반 너비 / 2))
            left_calc_percent = (black_start_index * key_width_pc) + (key_width_pc / 2) - (black_key_width_pc / 2)

            black_keys_html += f"""
            <div class="key black-key {'highlighted-black' if is_highlighted else ''}" style="left: {left_calc_percent}%; width: {black_key_width_pc}%; ">
            </div>
            """
